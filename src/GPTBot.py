...
class QTChatBot():
    ...
    def start(self):
        ...
        while not rospy.is_shutdown() and not self.finish:            
            if i == 0:
                self.audioPlay_pub.publish("custom_sounds/message_sound")
            if i >= 10:
                break
            ...
            try:    
                recognize_result = self.recognizeQuestion(language=self.defaultlanguage, options='', timeout=10)                        
